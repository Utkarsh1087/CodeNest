...
function FileContextProvider({ children }: { children: ReactNode }) {
    ...
    const handleJoinAccepted = useCallback(
        ({ initialState }: { initialState?: any }) => {
            if (initialState?.fileStructure) {
                setFileStructure(initialState.fileStructure)
                // Also open the first file if available
                const firstFile = initialState.fileStructure.children?.find((item: any) => item.type === "file")
                if (firstFile) {
